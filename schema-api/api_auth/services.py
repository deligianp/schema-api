...
from util.exceptions import ApplicationErrorHelper, ApplicationNotFoundError, ApplicationValidationError, \
    ApplicationError, ApplicationDuplicateError, ApplicationInvalidTokenError, ApplicationInactiveTokenError, \
    ApplicationExpiredTokenError
...
class ApiTokenService(BaseService):
    ...
    def authenticate(token: str) -> Union[Tuple[AuthEntity, None], Tuple[None, Participation]]:
        ...
        except ApiToken.DoesNotExist as atdne:
            raise ApplicationInvalidTokenError from atdne
        if not api_token.is_active:
            raise ApplicationInactiveTokenError('API token has been invalidated')
        if api_token.expiry < timezone.now():
            raise ApplicationExpiredTokenError('API token has expired')
