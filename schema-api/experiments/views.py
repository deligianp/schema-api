...
from experiments.services import ExperimentService, ExperimentTaskService, ExperimentWorkflowService
from util.exceptions import ApplicationValidationError, ApplicationNotFoundError
from workflows.serializers import WorkflowsBasicListSerializer
...
class ExperimentWorkflowsAPIView(APIView):
    authentication_classes = [ApiTokenAuthentication] if settings.USE_AUTH else []
    permission_classes = [IsAuthenticated, IsUser, IsActive, IsContextMember] if settings.USE_AUTH else []

    @extend_schema(
        summary='Set experiment workflows',
        description='Set experiment workflows by workflow UUID',
        tags=['Experiments'],
        request=True,
        examples=[
            OpenApiExample(
                'set-experiment-no-workflows',
                summary='Set experiment to have no workflows',
                value=[],
                request_only=True,
                response_only=False,
            ),
            OpenApiExample(
                'set-experiment-workflows',
                summary='Set experiment workflows',
                value=[
                    "551ec417-e034-4c3e-811b-69cba5cb377a",
                    "4883e205-274c-4f16-8fb6-b46199d759a5",
        ...
        responses={
            202: OpenApiResponse(
                description='Experiment was found and workflow list was updated',
                response=TasksBasicListSerializer,
                examples=[
                    OpenApiExample(
                        'updated-experiment-workflows-list',
                        summary='Updated experiment workflows list',
                        value=[
                            {
                                "uuid": "551ec417-e034-4c3e-811b-69cba5cb377a",
                                "name": "foo_task",
                    ...
                    OpenApiExample(
                        'updated-experiment-workflows-list-with-no-tasks',
                        summary='Updated experiment workflows list with no tasks',
                        value=[],
                        request_only=False,
                        response_only=True,
                    )
            ...
            404: OpenApiResponse(
                description='Either user is not a context participant, experiment not found in context or a referenced '
                            'workflows does not exist in context'
            )
        }
    )
    def put(self, request, username: str, name: str):
        ...
        # if not request.data:
        #     raise ApplicationValidationError('No workflow UUIDs were provided in request body')

        workflow_uuids = set(request.data)
        workflow_service = WorkflowService(context=request.context, user=creator)

        workflows = workflow_service.get_workflows().filter(uuid__in=workflow_uuids)
        if len(workflow_uuids) != len(workflows):
            indicative_uuid = workflow_uuids.difference(set(str(w.uuid) for w in workflows)).pop()

            raise ApplicationNotFoundError(f'No workflow with UUID "{indicative_uuid}" exists in context '
                                           f'"{request.context.name}"')

        experiment_service = ExperimentService(request.context)
        experiment = experiment_service.retrieve_experiment(name, creator)

        experiment_workflow_service = ExperimentWorkflowService(experiment)
        experiment_workflow_service.set_workflows(workflows)

        # TODO:
        output_serializer = WorkflowsBasicListSerializer(experiment_workflow_service.get_workflows(), many=True)
        return Response(status=status.HTTP_202_ACCEPTED, data=output_serializer.data)
    ...
    def get(self, request, username: str, name: str):
        ...
        workflows = experiment_workflow_service.get_workflows()
        output_serializer = WorkflowsBasicListSerializer(workflows, many=True)
        return Response(status=status.HTTP_200_OK, data=output_serializer.data)
